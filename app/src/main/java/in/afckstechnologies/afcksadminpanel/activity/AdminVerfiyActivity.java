...
public class AdminVerfiyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_verfiy);
    ...
    private class smspassAvailable extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            ...
            mProgressDialog = new ProgressDialog(AdminVerfiyActivity.this);
...
//
    ...
    private class userAvailable extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            WebClient serviceAccess = new WebClient();
            Log.i("json", "json" + jsonObj);
            trainerResponse = serviceAccess.SendHttpPost(Config.URL_AVAILABLEADMINPANEL, jsonObj);
...
// Process line...
    ...
    private class submitData extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            jsonObj = new JSONObject() {
                {
                    try {
                        put("id", user_id);
                        put("adminpanel_mobile_deviceid", deviceId);
            ...
            Log.i("json", "json" + jsonObj);
            trainerResponse = serviceAccess.SendHttpPost(Config.URL_UPDATEADMINPANELDETAILS, jsonObj);
            Log.i("resp", "centerListResponse" + trainerResponse);
        ...
        protected void onPostExecute(Void args) {
            if (status) {
                mProgressDialog.dismiss();
                try {
                    prefEditor.putString("adminpanel_user_id", user_id);
                    prefEditor.putString("phone_no", phoneno);
                    prefEditor.putString("adminpanel_mobile_deviceid", deviceId);
                    prefEditor.putString("adminpanel_user_name", jsonObject.getString("trainer_name"));
                    prefEditor.commit();
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                ...
                Toast.makeText(getApplicationContext(), msg, Toast.LENGTH_LONG).show();
                finish();
                Intent intent = new Intent(AdminVerfiyActivity.this, AdminPanelDashboardActivity.class);
